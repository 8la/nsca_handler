...
module MyOrganization
  class NSCAHandler < Chef::Handler
    ...
    def report
      ...
      host_name= Chef::Config['nagios_hostname'] || run_status.node[:hostname]
      ...
      Chef::Log.info(`echo \"#{msg_string}\" | #{@send_nsca_binary} -H #{node[:chef_handler][:nsca_server]} -p #{node[:chef_handler][:nsca_port]}`)
    end
  end
end
